...
var StdStringCoder = class extends Coder {
  ...
  decode(data, offset) {
    const len = data.slice(16, 24);
    const length = (0, import_math9.bn)(new U64Coder().decode(len, 0)[0]).toNumber();
...
var AbiCoder = class {
  ...
  static getCoderImpl(resolvedAbiType) {
    ...
    if (resolvedAbiType.type === "str") {
      throw new import_errors17.FuelError(
        import_errors17.ErrorCode.INVALID_DATA,
...
var FunctionFragment = class {
  ...
  static getFunctionSelector(functionSignature) {
    const hashedFunctionSignature = (0, import_ethers6.sha256)((0, import_crypto.bufferFromString)(functionSignature, "utf-8"));
    return (0, import_math10.bn)(hashedFunctionSignature.slice(0, 10)).toHex(8);
  }
  #isInputDataPointer() {
  ...
  decodeArguments(data) {
    const bytes = (0, import_ethers6.getBytesCopy)(data);
    const nonEmptyInputs = this.jsonFn.inputs.filter(
      (x) => findOrThrow(this.jsonAbi.types, (t) => t.typeId === x.type).type !== "()"
    );
...
var Interface = class {
  ...
  decodeLog(data, logId, receiptId) {
    ...
    return AbiCoder.decode(this.jsonAbi, loggedType, (0, import_ethers7.getBytesCopy)(data), 0);
  }
