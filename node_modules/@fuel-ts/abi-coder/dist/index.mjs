...
// src/coders/array.ts
import { ErrorCode as ErrorCode3 } from "@fuel-ts/errors";
...
function findOrThrow(arr, predicate, throwFn = () => {
  throw new FuelError2(ErrorCode2.ELEMENT_NOT_FOUND, "Element not found in the array.");
}) {
...
var ArrayCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      this.throwError(ErrorCode3.ENCODE_ERROR, `Expected array value.`);
    }
    if (this.length !== value.length) {
      this.throwError(ErrorCode3.ENCODE_ERROR, `Types/values length mismatch.`);
    }
...
// src/coders/b256.ts
import { ErrorCode as ErrorCode4 } from "@fuel-ts/errors";
import { bn as bn2, toHex } from "@fuel-ts/math";
...
var B256Coder = class extends Coder {
  ...
  encode(value) {
    let encodedValue;
    try {
      encodedValue = getBytesCopy2(value);
    } catch (error) {
      this.throwError(ErrorCode4.ENCODE_ERROR, `Invalid ${this.type}.`);
    }
  ...
  decode(data, offset) {
    ...
    if (bytes.length !== 32) {
      this.throwError(ErrorCode4.DECODE_ERROR, `'Invalid size for b256'.`);
    }
...
// src/coders/b512.ts
import { ErrorCode as ErrorCode5 } from "@fuel-ts/errors";
import { bn as bn3, toHex as toHex2 } from "@fuel-ts/math";
...
var B512Coder = class extends Coder {
  ...
  encode(value) {
    let encodedValue;
    try {
      encodedValue = getBytesCopy3(value);
    } catch (error) {
      this.throwError(ErrorCode5.ENCODE_ERROR, `Invalid ${this.type}.`);
    }
  ...
  decode(data, offset) {
    ...
    const decoded = bn3(bytes);
    if (decoded.isZero()) {
      bytes = new Uint8Array(64);
    }
    if (bytes.length !== 64) {
      this.throwError(ErrorCode5.DECODE_ERROR, `Invalid size for b512.`);
    }
    return [toHex2(bytes, 64), offset + 64];
  }
};

// src/coders/boolean.ts
import { ErrorCode as ErrorCode6 } from "@fuel-ts/errors";
...
var BooleanCoder = class extends Coder {
  ...
  encode(value) {
    ...
    if (!isTrueBool) {
      this.throwError(ErrorCode6.ENCODE_ERROR, `Invalid boolean value.`);
    }
    return toBytes2(value ? 1 : 0, 8);
  }
  decode(data, offset) {
    const bytes = bn4(data.slice(offset, offset + 8));
    if (bytes.isZero()) {
      return [false, offset + 8];
    }
    if (!bytes.eq(bn4(1))) {
      this.throwError(ErrorCode6.DECODE_ERROR, `Invalid boolean value.`);
    }
...
// src/coders/enum.ts
import { ErrorCode as ErrorCode7, FuelError as FuelError3 } from "@fuel-ts/errors";
...
var EnumCoder = class extends Coder {
  ...
  encode(value) {
    ...
    if (!caseKey) {
      throw new FuelError3(ErrorCode7.INVALID_DECODE_VALUE, "A field for the case must be provided.");
    }
    if (empty.length !== 0) {
      throw new FuelError3(ErrorCode7.INVALID_DECODE_VALUE, "Only one field must be provided.");
    }
...
import { ErrorCode as ErrorCode8 } from "@fuel-ts/errors";
...
var NumberCoder = class extends Coder {
  ...
  encode(value) {
    ...
    try {
      ...
      this.throwError(ErrorCode8.ENCODE_ERROR, `Invalid ${this.baseType}.`);
    }
    if (bytes.length > this.length) {
      this.throwError(ErrorCode8.ENCODE_ERROR, `Invalid ${this.baseType}, too many bytes.`);
    }
    return toBytes3(bytes, 8);
  }
  decode(data, offset) {
    let bytes = data.slice(offset, offset + 8);
    bytes = bytes.slice(8 - this.length, 8);
    return [toNumber2(bytes), offset + 8];
  }
};

// src/coders/string.ts
import { ErrorCode as ErrorCode9 } from "@fuel-ts/errors";
...
var StringCoder = class extends Coder {
  ...
  encode(value) {
    if (this.length !== value.length) {
      this.throwError(ErrorCode9.ENCODE_ERROR, `Value length mismatch during encode.`);
    }
    const encoded = toUtf8Bytes(value);
    const padding = new Uint8Array(this.#paddingLength);
    return concat3([encoded, padding]);
  }
  decode(data, offset) {
...
import { ErrorCode as ErrorCode10 } from "@fuel-ts/errors";
...
// src/coders/tuple.ts
import { ErrorCode as ErrorCode11 } from "@fuel-ts/errors";
var TupleCoder = class extends Coder {
  ...
  encode(value) {
    if (this.coders.length !== value.length) {
      this.throwError(ErrorCode11.ENCODE_ERROR, `Types/values length mismatch.`);
    }
...
// src/coders/vec.ts
import { ErrorCode as ErrorCode12 } from "@fuel-ts/errors";
...
var VecCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      this.throwError(ErrorCode12.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
  ...
  decode(data, offset) {
    ...
    const length = bn5(new U64Coder().decode(len, 0)[0]).toNumber();
...
// src/interface.ts
import { ErrorCode as ErrorCode18, FuelError as FuelError7 } from "@fuel-ts/errors";
import { getBytesCopy as getBytesCopy5 } from "ethers";

// src/abi-coder.ts
import { ErrorCode as ErrorCode16, FuelError as FuelError5 } from "@fuel-ts/errors";

// src/coders/byte.ts
import { ErrorCode as ErrorCode13 } from "@fuel-ts/errors";
...
var ByteCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      this.throwError(ErrorCode13.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
    const pointer = new U64Coder().encode(BASE_VECTOR_OFFSET);
  ...
  decode(data, offset) {
    const len = data.slice(16, 24);
    const length = bn6(new U64Coder().decode(len, 0)[0]).toNumber();
...
import { ErrorCode as ErrorCode14 } from "@fuel-ts/errors";
var RawSliceCoder = class extends Coder {
  ...
  encode(value) {
    if (!Array.isArray(value)) {
      this.throwError(ErrorCode14.ENCODE_ERROR, `Expected array value.`);
    }
    const parts = [];
...
// src/coders/stdString.ts
import { bn as bn7 } from "@fuel-ts/math";
import { concat as concat5 } from "@fuel-ts/utils";
import { toUtf8Bytes as toUtf8Bytes2, toUtf8String as toUtf8String2 } from "ethers";
...
import { ErrorCode as ErrorCode15, FuelError as FuelError4 } from "@fuel-ts/errors";
...
var AbiCoder = class {
  ...
  static getCoderImpl(resolvedAbiType) {
    ...
    if (resolvedAbiType.type === "str") {
      throw new FuelError5(
        ErrorCode16.INVALID_DATA,
        "String slices can not be decoded from logs. Convert the slice to `str[N]` with `__to_str_array`"
      );
    }
    throw new FuelError5(
      ErrorCode16.CODER_NOT_FOUND,
...
import { ErrorCode as ErrorCode17, FuelError as FuelError6 } from "@fuel-ts/errors";
import { bn as bn8 } from "@fuel-ts/math";
import { sha256, getBytesCopy as getBytesCopy4 } from "ethers";
var FunctionFragment = class {
  ...
  static getFunctionSelector(functionSignature) {
    ...
    return bn8(hashedFunctionSignature.slice(0, 10)).toHex(8);
  }
  #isInputDataPointer() {
  ...
  static verifyArgsAndInputsAlign(args, inputs, abi) {
    ...
    throw new FuelError6(ErrorCode17.ABI_TYPES_AND_VALUES_MISMATCH, errorMsg);
  }
  decodeArguments(data) {
    const bytes = getBytesCopy4(data);
    const nonEmptyInputs = this.jsonFn.inputs.filter(
      (x) => findOrThrow(this.jsonAbi.types, (t) => t.typeId === x.type).type !== "()"
    );
...
var Interface = class {
  ...
  getFunction(nameOrSignatureOrSelector) {
    ...
    throw new FuelError7(
      ErrorCode18.FUNCTION_NOT_FOUND,
  ...
  decodeFunctionData(functionFragment, data) {
    ...
    if (!fragment) {
      throw new FuelError7(ErrorCode18.FRAGMENT_NOT_FOUND, "Fragment not found.");
    }
  ...
  decodeLog(data, logId, receiptId) {
    ...
    return AbiCoder.decode(this.jsonAbi, loggedType, getBytesCopy5(data), 0);
  }
  ...
  encodeConfigurable(name, value) {
    const configurable = findOrThrow(
      ...
      () => {
        throw new FuelError7(
          ErrorCode18.CONFIGURABLE_NOT_FOUND,
