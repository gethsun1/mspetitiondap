...
var getContractCallScript = (functionScopes, maxInputs) => new ScriptRequest(
  ...
  (contractCalls) => {
    ...
    for (let i = 0; i < TOTAL_CALLS; i += 1) {
      ...
      paramOffsets.push({
        gasForwardedOffset: call2.gas ? segmentOffset + import_abi_coder2.WORD_SIZE + import_abi_coder2.ASSET_ID_LEN + import_abi_coder2.CONTRACT_ID_LEN + import_abi_coder2.WORD_SIZE : 0,
        amountOffset: segmentOffset,
        assetIdOffset: segmentOffset + import_abi_coder2.WORD_SIZE,
        callDataOffset: segmentOffset + import_abi_coder2.WORD_SIZE + import_abi_coder2.ASSET_ID_LEN
      });
...
var BaseInvocationScope = class {
  ...
  txParams(txParams) {
    ...
    request.gasPrice = (0, import_math3.bn)(txParams.gasPrice || request.gasPrice);
  ...
  async call() {
    ...
    if (gasUsed.gt((0, import_math3.bn)(transactionRequest.gasLimit))) {
      throw new import_errors5.FuelError(
  ...
  async getTransactionId(chainId) {
    ...
    const transactionRequest = await this.getTransactionRequest();
    return (0, import_hasher.hashTransaction)(transactionRequest, chainIdToHash);
  }
};
