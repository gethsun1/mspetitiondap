...
import { WORD_SIZE as WORD_SIZE2, U64Coder, B256Coder, ASSET_ID_LEN as ASSET_ID_LEN2, CONTRACT_ID_LEN as CONTRACT_ID_LEN2 } from "@fuel-ts/abi-coder";
...
var scriptResultDecoder = (contractId, isOutputDataHeap) => (result) => {
  ...
  return receipts.filter(({ type }) => isReturnType(type)).flatMap((receipt, index, filtered) => {
    ...
    if (receipt.type === ReceiptType4.ReturnData) {
      const encodedScriptReturn = getBytesCopy2(receipt.data);
      if (isOutputDataHeap && isReturnType(filtered[index + 1]?.type)) {
        const nextReturnData = filtered[index + 1];
        return concat2([encodedScriptReturn, getBytesCopy2(nextReturnData.data)]);
      }
...
var getContractCallScript = (functionScopes, maxInputs) => new ScriptRequest(
  ...
  (contractCalls) => {
    ...
    for (let i = 0; i < TOTAL_CALLS; i += 1) {
      ...
      paramOffsets.push({
        gasForwardedOffset: call2.gas ? segmentOffset + WORD_SIZE2 + ASSET_ID_LEN2 + CONTRACT_ID_LEN2 + WORD_SIZE2 : 0,
        amountOffset: segmentOffset,
        assetIdOffset: segmentOffset + WORD_SIZE2,
        callDataOffset: segmentOffset + WORD_SIZE2 + ASSET_ID_LEN2
      });
      ...
      const args = getBytesCopy2(call2.data);
...
var BaseInvocationScope = class {
  ...
  txParams(txParams) {
    ...
    request.gasPrice = bn3(txParams.gasPrice || request.gasPrice);
  ...
  async call() {
    ...
    if (gasUsed.gt(bn3(transactionRequest.gasLimit))) {
      throw new FuelError4(
