...
var formatScriptDataForTransferringToContract = (hexelifiedContractId, amountToTransfer, assetId) => {
  const numberCoder = new import_abi_coder.U64Coder();
  const encoded = numberCoder.encode(new import_math.BN(amountToTransfer).toNumber());
  const scriptData = Uint8Array.from([
    ...(0, import_ethers.getBytesCopy)(hexelifiedContractId),
    ...encoded,
    ...(0, import_ethers.getBytesCopy)(assetId)
  ]);
  return scriptData;
};
...
var Account = class extends import_interfaces.AbstractAccount {
  ...
  async fund(request, quantities, fee) {
    (0, import_providers.addAmountToAsset)({
      amount: fee,
      assetId: import_configs.BaseAssetId,
   ...
   */
  async prepareTransferTxRequest(destination, amount, assetId = import_configs.BaseAssetId, txParams = {}) {
  ...
  async transferToContract(contractId, amount, assetId = import_configs.BaseAssetId, txParams = {}) {
    ...
    const { maxFee, requiredQuantities } = await this.provider.getTransactionCost(request, [
      { amount: (0, import_math2.bn)(amount), assetId: String(assetId) }
    ]);
    await this.fund(request, requiredQuantities, maxFee);
    return this.sendTransaction(request);
  }
  ...
  async withdrawToBaseLayer(recipient, amount, txParams = {}) {
    const recipientDataArray = (0, import_ethers2.getBytesCopy)(
      "0x".concat(recipient.toHexString().substring(2).padStart(64, "0"))
    );
    const amountDataArray = (0, import_ethers2.getBytesCopy)(
      "0x".concat((0, import_math2.bn)(amount).toHex().substring(2).padStart(16, "0"))
    );
    const script = new Uint8Array([
      ...(0, import_ethers2.getBytesCopy)(import_providers.withdrawScript.bytes),
      ...recipientDataArray,
      ...amountDataArray
    ]);
    const { maxGasPerTx } = this.provider.getGasConfig();
...
async function decryptKeystoreWallet(jsonWallet, password) {
  ...
  const privateKey = (0, import_ethers3.hexlify)(buffer);
  return privateKey;
}
...
var import_signer3 = require("@fuel-ts/signer");
  ...
  new Promise(async (resolve) => {
    ...
    if (!chainConfigPath) {
      ...
      if (!process.env.GENESIS_SECRET) {
        const pk = import_signer3.Signer.generatePrivateKey();
        const signer = new import_signer3.Signer(pk);
        process.env.GENESIS_SECRET = (0, import_ethers4.hexlify)(pk);
        chainConfig = {
          ...defaultChainConfig,
          initial_state: {
            ...
            coins: [
              ...defaultChainConfig.initial_state.coins,
              {
                ...
                amount: (0, import_math3.toHex)(1e9),
                asset_id: import_configs2.BaseAssetId
              }
            ]
          }
        };
      }
      await import_promises.default.writeFile(tempChainConfigFilePath, JSON.stringify(chainConfig), "utf8");
