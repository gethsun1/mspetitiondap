...
import { coinQuantityfy, ScriptTransactionRequest as ScriptTransactionRequest2 } from "@fuel-ts/providers";
var seedTestWallet = async (wallet, quantities) => {
  const genesisWallet = new WalletUnlocked(
    process.env.GENESIS_SECRET || randomBytes2(32),
    wallet.provider
  );
...
import { BaseAssetId as BaseAssetId2 } from "@fuel-ts/address/configs";
import { toHex } from "@fuel-ts/math";
import { Provider } from "@fuel-ts/providers";
import { Signer as Signer3 } from "@fuel-ts/signer";
  ...
  new Promise(async (resolve) => {
    ...
    if (!chainConfigPath) {
      ...
      if (!process.env.GENESIS_SECRET) {
        const pk = Signer3.generatePrivateKey();
        const signer = new Signer3(pk);
...
var generateWallets = async (count, provider) => {
  ...
  for (let i = 0; i < count; i += 1) {
    const wallet = await generateTestWallet(provider, [[1e3, BaseAssetId2]]);
    wallets.push(wallet);
  }
  return wallets;
};
